  ...
  for (var url in swagger.paths) {
    ...
    for (var method in path || {}) {
      ...
      for (var p = 0; p < parameters.length; p++) {
        ...
        paramVar = options.camelCaseServiceParams & param.in != "header" ? paramVar.charAt(0).toLowerCase() + paramVar.slice(1) : paramVar;
        var paramDescriptor = {
          paramName: options.camelCaseServiceParams & param.in != "header" ? param.name.charAt(0).toLowerCase() + param.name.slice(1) : param.name,
          paramIn: param.in,
          paramVar: paramVar,
